...
class DocWriter:
    ...
    def tagit(self, token:cindex.Token) -> str:
        ...
        kind = token.kind
        text = escape(spell)
        if spell in {'#', 'ifdef', 'undef', 'define', 'include', 'import', 'ifndef', 'elif', 'endif', 'pragma', 'error'}:
            if spell == 'include':
                self.include = True
...
def do_tags(arguments:List[str], source_file:str, compiler:str) -> DocWriter:
    ...
    for x in ['printf', 'fprintf', 'memcpy', 'memset', 'memcmp', 'memchr', 'memmem', 'malloc', 'realloc', 'free', 'calloc']:
        f(x)
    for x in ['DARRAY_DECLS_ONLY', 'DARRAY_HEADER_H']:
        m(x)
    ...
    if good_tag(cursor):
        ...
        if cursor.kind == CursorKind.ENUM_CONSTANT_DECL:
            if not cursor.semantic_parent.spelling:
                kind = Kinds.anonenum
        if cursor.spelling not in identifiers:
...
def run(file:str, outfile:str, dep_file:Optional[str], cflags:List[str]) -> None:
