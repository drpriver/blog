...
def main() -> None:
    ...
    pages = [p for p in os.listdir('docs/games') if p != 'index.html' and p.endswith('.html') and p not in published_pages]

    for page in pages:
        with open('docs/games/'+page, 'r') as fp:
            text = fp.read()
        ...
        link.text = 'https://www.davidpriver.com/games/'+page
        author = SubElement(item, 'author')
        author.text = 'david@davidpriver.com'

    with open('games-feed.xml', 'wb') as fp:
